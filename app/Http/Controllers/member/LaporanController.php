...
class LaporanController extends Controller
{
    ...
    public function averageYearOfKas($tahun = null)
    {
        $data_debit = Cash::select(\DB::raw('SUM(c_jumlah) as jumlah_debit'),
            \DB::raw("DATE_FORMAT(c_tanggal, '%M %Y') as month"), 'c_jenis')
            ->where('c_iduser', Auth::user()->id)
        ...
        $total_debit        = 0;
        $total_credit       = 0;
        $count_debit        = count($data_debit);
        $count_credit       = count($data_credit);

        if ($count_debit > 0) {
            foreach ($data_debit as $key => $value) {
                $total_debit += $value->jumlah_debit;
            }
        }

        if ($count_credit > 0) {
            foreach ($data_credit as $key => $value) {
                $total_credit += $value->jumlah_kredit;
            }
        }

        $average_debit = $total_debit/$count_debit;
        $average_credit = $total_credit/$count_credit;

        return Response::json(['count_debit'=>$count_debit, 'count_credit'=>$count_credit, 'total_debit'=>$total_debit, 'total_credit'=>$total_credit,'average_debit'=>$average_debit, 'average_credit'=>$average_credit]);
    }
    ...
    public function averageYearOfBank($tahun = null)
    {
        $data_debit = Cash::select(\DB::raw('SUM(c_jumlah) as jumlah_debit'),
            \DB::raw("DATE_FORMAT(c_tanggal, '%M %Y') as month"), 'c_jenis')
            ->where('c_iduser', Auth::user()->id)
    ...
    public function averageMonthOfKas($bulan = null)
    {
        $month = explode(" ", $bulan);
        $t = $month[1].'-'.Helper::month($month[0]).'-'.'1';
        $from = date('Y-m-d', strtotime($t));
    ...
    public function averageMonthOfBank($bulan = null)
    {
        $month = explode(" ", $bulan);
        $t = $month[1].'-'.Helper::month($month[0]).'-'.'1';
        $from = date('Y-m-d', strtotime($t));
